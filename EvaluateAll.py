    ...
    with torch.no_grad():
        for images, true_masks, pth in zip(image_data, mask_data, imgs_paths):
            ...
            if pq[0] != 0:
                ...
                dice = 0
                aji = 0
            if ('All104586' in args.dataset_name):
                im = os.path.join('/home/ntorbati/STORAGE/NucleiAnalysis/tif/BigTestFolder', pth)
                im = cv2.cvtColor(cv2.imread(im), cv2.COLOR_BGR2RGB)[0:256,0:256]
                plt.imshow(im)
                plt.savefig(f'results/{pth}_Org_{args.dataset_name}_{args.model}.png', dpi = 300)

                im = np.copy(pred_inst)[0:256,0:256]
                im1 = 0 * np.copy(im)
                cnt1 = []
                for i in range(1,np.max(im)+1):
                    im1[im == i] = 1

                    im1 = (im1 > 0).astype(np.uint8) * 255

                    # Find contours
                    contours, _ = cv2.findContours(im1, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                    cnt1.append(contours)

                # Create an RGB image

                # Create an RGB image to draw borders on (optional)
                border_img = cv2.cvtColor(255*(im > 0).astype(np.uint8), cv2.COLOR_GRAY2BGR)
                for cnt in cnt1:
                    cv2.drawContours(border_img, cnt, -1, (255, 0, 0), 2)


                plt.imshow(border_img)
                plt.savefig(f'results/{pth}_Pred_{args.dataset_name}_{args.model}.png', dpi = 300)

                im = np.copy(gt)[0:256,0:256]
                im1 = 0 * np.copy(im)
                cnt1 = []
                for i in range(1,np.max(im)+1):
                    im1[im == i] = 1
                ...
                plt.imshow(border_img)
                plt.savefig(f'results/{pth}_GT_{args.dataset_name}_{args.model}.png', dpi = 300)
...
dataset_tags = ['puma','pan-cancer-nuclei-seg', 'monuseg', 'cpm17', 'tnbc',
                'cryonuseg', 'nuinsseg', 'consep'
                , 'monusac', 'data science bowl']
...
if __name__ == "__main__":
    ...
    for model in models:
        ...
        datasets_names = np.sort(os.listdir(data_path))
        results_dict_all = []
        for dataset_name in datasets_names:
           for ind in [2,3,4,5,6,7,8,9,10]:
                if ('All' + str(ind)) in dataset_name:
                    print(dataset_name)
                    results_dict = {}
                    ...
                    if args.model == 'hovernext' or args.model == 'acs':
                        ...
                        sub_fld = np.sort([flds for flds in os.listdir(checkpoint_path) if '_None' in flds])
                        checkpoint_path = os.path.join(checkpoint_path, sub_fld[-1] ,'checkpoints', 'model_best.pth')
        ...
        np.save('/home/ntorbati/PycharmProjects/NucleiAnalysis/Test_Results/All' + model_name + '.npy', results_dict_all)
        np.save('/home/ntorbati/PycharmProjects/NucleiAnalysis/Test_Results/All' + model_name + 'overall.npy', overal)
        print(overal)
